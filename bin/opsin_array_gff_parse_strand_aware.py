...
SAMPLE_ID = ""
HAP_NAME = ""
UPSTREAM_FLANK = 12010
DOWNSTREAM_FLANK = 2393

def initialize_genes_tsv(genes_tsv_path):
    """Create output TSV with header so empty inputs still emit output."""
    with open(genes_tsv_path, 'w') as out:
        out.write("sample_id\tsex\thap_name\tcontig\tstart_position\tend_position\ttype\tgene_rank\n")


def compute_gene_window(start_pos, end_pos, is_reverse_orientation):
    """
    Compute strand-aware extraction window around exon5 coordinates.
    Forward orientation keeps original flank behavior:
      start - UPSTREAM_FLANK, end + DOWNSTREAM_FLANK
    Reverse orientation swaps flanks in genomic coordinates so the same
    biological upstream/downstream context is captured.
    """
    if is_reverse_orientation:
        window_start = max(1, start_pos - DOWNSTREAM_FLANK)
        window_end = end_pos + UPSTREAM_FLANK
    else:
        window_start = max(1, start_pos - UPSTREAM_FLANK)
        window_end = end_pos + DOWNSTREAM_FLANK
                ...
                try:
                    ...
                    with open(GENES_TSV_PATH, 'a') as out:
                        ...
                        for rank, gene in enumerate(all_exon5[:2], 1):
                            # Without an LCR anchor, use the hit strand for orientation.
                            is_reverse = gene.get('strand') == '-'
                            start_pos, end_pos = compute_gene_window(
                                gene['position'],
                                gene['end_position'],
                                is_reverse
                            )
    ...
    if primary_structure and additional_genes:
        ...
        if GENES_TSV_PATH and primary_contig['exon5_count'] == 1 and len(additional_genes) >= 1:
            try:
                ...
                is_reverse = add_gene.get('strand') == '-'
                start_position, end_position = compute_gene_window(
                    add_gene['position'],
                    add_gene['end_position'],
                    is_reverse
                )
                gene2 = {
                    'type': add_gene['type'],
...
def build_array_structure(exon5_list, is_reverse_orientation):
    ...
    second_gene = None
    if len(exon5_list) >= 1:
        first_start, first_end = compute_gene_window(
            exon5_list[0]['position'],
            exon5_list[0]['end_position'],
        ...
        first_gene = {
            ...
            'end_position': first_end
        }
    if len(exon5_list) >= 2:
        second_start, second_end = compute_gene_window(
            exon5_list[1]['position'],
            exon5_list[1]['end_position'],
            is_reverse_orientation
        )
        second_gene = {
...
if __name__ == "__main__":
    ...
    HAP_NAME = hap_name
    SEX = sex
    initialize_genes_tsv(GENES_TSV_PATH)
